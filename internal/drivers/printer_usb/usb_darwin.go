...
// darwinUSBDevice implements USBDevice for macOS using gousb
type darwinUSBDevice struct {
	device   *gousb.Device
...
// findUSBPrinter finds and opens a USB ESC/POS printer on macOS
func findUSBPrinter(config Config) (USBDevice, error) {
	...
	// Note: macOS handles kernel driver detachment automatically
	// No need to call SetAutoDetach like on Linux
...
func (d *darwinUSBDevice) Write(data []byte) (int, error) {
	d.mu.Lock()
	if d.closed {
		d.mu.Unlock()
...
func (d *darwinUSBDevice) Read(buffer []byte) (int, error) {
	d.mu.Lock()
	if d.closed {
		d.mu.Unlock()
		return 0, fmt.Errorf("device closed")
...
func (d *darwinUSBDevice) Close() error {
	d.mu.Lock()
	defer d.mu.Unlock()
...
func (d *darwinUSBDevice) GetInfo() (vendorID, productID uint16, serial string, err error) {
	return d.vendorID, d.productID, d.serial, nil
}

// Reset resets the USB device
func (d *darwinUSBDevice) Reset() error {
	d.mu.Lock()
	defer d.mu.Unlock()
