...
// This is a standalone renderer that can be used by the USB printer driver
func renderDocument(doc *printer.PrintDocument) ([]byte, error) {
	buf := &bytes.Buffer{}
	...
	for _, section := range doc.Sections {
		if err := renderSection(buf, section); err != nil {
			return nil, err
		}
	}
...
func renderSection(buf *bytes.Buffer, section printer.Section) error {
	for _, element := range section.Elements {
		switch element.Type {
		case printer.ElementTypeLine:
			if element.Line != nil {
				renderLine(buf, *element.Line)
			}
		case printer.ElementTypeBarcode:
			if element.Barcode != nil {
				renderBarcode(buf, *element.Barcode)
			}
		case printer.ElementTypeQRCode:
			if element.QRCode != nil {
				renderQRCode(buf, *element.QRCode)
			}
		case printer.ElementTypeImage:
			// Image rendering is complex, not implemented yet
			// Images require bitmap conversion and ESC/POS raster commands
		}
	}
...
func renderLine(buf *bytes.Buffer, line printer.Line) {
	// Set alignment
...
func renderRun(buf *bytes.Buffer, run printer.Run) {
	// Set bold
	if run.Style.Bold {
...
func renderBarcode(buf *bytes.Buffer, barcode printer.Barcode) {
	// Set barcode height
	// GS h n (default 162)
...
func renderQRCode(buf *bytes.Buffer, qrcode printer.QRCode) {
	// QR code model
